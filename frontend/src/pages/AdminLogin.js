...
const AdminLogin = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const postData = async () => {
      try {
        const response = await axios.post(`/api/admins/login`, adminData);
        if (response) {
  ...
  return (
    <div>
      ...
      <form className="w-4/5 md:w-2/5 mx-auto mt-20">
        ...
        <button
          className="flex items-center gap-3 font-bold text-2xl bg-primary text-white rounded px-10 py-3 mt-10"
